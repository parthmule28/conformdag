...
| [C03](prompts/C03-check-catalogue.md) | review | `docs/c03-authoritative-check-catalogue` (PR pending) | `mise run check`: 527 passed, 19 deselected; `mise run test:coverage`: 90.98%; 546 collected; `mise run schema --check`; `mise run build`; focused registry/evaluator/fixing/CLI/policy/round-trip suite: 148 passed; `git diff --check` | One authoritative `CheckSpec` catalogue with 15 entries and exactly 12 executable evaluator-backed entries; evaluator compatibility imports are cycle-safe and internal routing uses a local registry helper; `fix_kind` vocabulary and explicit `FIXERS` preserved; semantic-only entries remain non-executable until C05; no scan, policy ID, finding, report, fix, or scaffold behavior changes |
