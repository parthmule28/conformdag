...
| [C05](prompts/C05-deterministic-checks.md) | review | `docs/c05-deterministic-checks` (PR pending) | Approved Native implementation from base `3a651fe8b516b18fb8de2994e76c262635ce7482`; focused modular-check/facade/fixing suite: 154 passed; `mise run check`: 575 passed, 2 skipped, 19 deselected; `mise run test:coverage`: 90.81% total coverage; `mise run schema --check`; `git diff --check`; Ruff, Pyright, policy-pack validation, dependency inventory, and the 80-case round-trip population passed; independent review of `3a651fe8b..df0e6d0` found no Critical/Important issues and assessed the branch ready to merge | `checks.common`, `checks.airflow.{metadata,scheduling,safety}`, and `checks.evaluate` now own deterministic checks; `conformdag.evaluator` is a compatibility facade; C03's catalogue and C04 boundaries remain authoritative; C06 application orchestration was not pulled forward; minor review documentation cleanups applied; awaiting PR creation and human merge authority |
